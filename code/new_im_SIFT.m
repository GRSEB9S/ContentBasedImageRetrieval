...
Jpegfile=dir('C:\Users\abhishek\Desktop\SIFT_and _SURF\new_im\*.jpg');
number1=length(Jpegfile);
a='C:\Users\abhishek\Desktop\SIFT_and _SURF\new_im\';
%keypoints_finals=cell(1,number1);
%files=cell(1,number1);
strtn=1;
for start=len:len+number1-1
    %display(Jpegfile(strtn).name);
    %display(strtn);
im = strcat(a,Jpegfile(strtn).name);im
strtn=strtn+1;
image=imread(im);
files{start+1}=(Jpegfile(strtn).name);
                     ...
                     if(image_extrema{octave,interval-1}(r,c)==255)%if we are at the keypoint
                         ...
                         for x=-8:1:8
                             for y=-8:1:8
                                     ...
                                     hist_orient(floor( (sample_orient_degrees)/10))=hist_orient(floor( (sample_orient_degrees)/10))+image_weight(r+x,c+y);
...
keypoints_finals{start+1}=(keypoints_final);
%files{start}= (im);
