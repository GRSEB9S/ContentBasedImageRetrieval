             ...
             for c=1:ncols
                flag=false;
                 ...
                 f(27)=dog_cell{octave,interval-1}(r,c);
                    ...
                    %Edge Check
%                      if(flag==true)
%                         Dxx=double(dog_cell{octave,interval}(r+1,c)+dog_cell{octave,interval}(r-1,c)-2*dog_cell{octave,interval}(r,c));
%                         Dyy=double(dog_cell{octave,interval}(r,c+1)+dog_cell{octave,interval}(r,c-1)-2*dog_cell{octave,interval}(r,c));
%                         Dxy=double(dog_cell{octave,interval}(r+1,c+1)+dog_cell{octave,interval}(r,c)-dog_cell{octave,interval}(r+1,c)-dog_cell{octave,interval}(r,c+1));
%                         Dxy=Dxy/4;
%                        trace=Dxx+Dyy;
%                        det=Dxx*Dyy-Dxy*Dxy;
%                        curvature_ratio=(trace*trace)/det;
%                        curvature_threshold=((5+1)^2)/5;
% 
%                        if(det<0 || curvature_ratio>curvature_threshold)
%                            image_extrema{octave,interval-1}(r,c)=0;
%                            number_keypoint=number_keypoint-1;
%                            reject=reject+1; 
%                            flag=false;
...
%hold off;
  %%
 %Extracting the keypoint descriptor
